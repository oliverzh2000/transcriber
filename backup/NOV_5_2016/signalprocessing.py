...
def read_WAV_file(filename):
    ...
    return struct.unpack(str(wave_file.getnframes()) + "h", samples)
...
def normalize(samples):
    ...
    return [sample / max_value for sample in samples]
...
'''samples = read_WAV_file("data/ten_period_440hz.WAV")
samples = normalize(samples)
write_txt_file("output.txt", samples)
...
N = len(samples)
initial_k = 3
for shift in range(140):
    k = initial_k + shift * 0.1
    freq_bin_mag = freq_bin_at(k, samples)
    transforms.append(freq_bin_mag)

write_txt_file("output1.txt", transforms)'''