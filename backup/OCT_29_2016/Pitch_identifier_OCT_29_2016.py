...
def record_chunk():
    ...
    stream.stop_stream()
    samples = unpack(CHUNK * "h", audio_data)
    max_value = max(samples)
    normalized_data = [float(frame)/max_value for frame in samples]
    return samples


def events():
    global done, width, height, identified_freq
...
print "framerate:", frame / (time.time() - prev_time)
pygame.quit()
sys.exit()